...
def translate(
    ...
    # 执行文件处理（含翻译文件操作）
    translate_path, msg = "", True

    # 执行文件操作失败
    if not translate_path:
        chat_history.append((full_query,f"执行翻译文件操作失败！失败原因：{msg}"))
        return chat_history, None

    # 构建点击可下载的链接
    download_html = f"""
    <div style="margin-top:10px;">
        <a href='/file={translate_path}' download='{os.path.basename(translate_path)}' 
    ...
    return chat_history, translate_path
