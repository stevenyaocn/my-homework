...
def translate_text(text, source_language: str = "英语", target_language: str = "中文"):
    """增强型翻译函数"""
    if not text.strip():
        return ""
    ...
    response = client.chat.completions.create(
        model=model,
        messages=[
            {"role": "system", "content": f"你是一位{target_language}翻译专家,尤其擅长将{source_language}翻译成{target_language}"},
...
def process_page(page, page_num, source_language, targe_language):
    """处理单页内容"""
    elements = []
    ...
    if text:
        translated = translate_text(text, source_language, targe_language)
    ...
    for table in tables:
        translated_table = [
            [Paragraph(translate_text(cell, source_language, targe_language).replace(' ', '&nbsp;'), simsun_style)
...
def process_pdf(input_pdf, output_pdf, source_language, target_language):
    """主处理流程"""
    ...
    with pdfplumber.open(input_pdf) as pdf:
        for page_num, page in enumerate(pdf.pages, start=1):
            story.extend(process_page(page, page_num, source_language=source_language, targe_language=target_language))
...
class PDFTranslator:
    ...
    def translate_pdf(self, pdf_file_path: str, source_language: str = "英文", target_language: str = "中文", output_file_path: str = None) -> str:
       return process_pdf(pdf_file_path, output_file_path, source_language, target_language)
